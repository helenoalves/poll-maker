package org.poll.maker.test.util;
...
public class WiserAssertions {
	...
	public WiserAssertions to(String to) {
		findFirstOrElseThrow(m -> m.getEnvelopeReceiver().equals(to), assertionError("No message to [{0}] found!", to));
		return this;
	}
	...
	public WiserAssertions withSubject(String subject) {
		...
		findFirstOrElseThrow(predicate, assertionError("No message with subject [{0}] found!", subject));
		return this;
	}

	public WiserAssertions containsSubject(String subject) {
		Predicate<WiserMessage> predicate = m -> unchecked(getMimeMessage(m)::getSubject).matches(subject);
		findFirstOrElseThrow(predicate, assertionError("No message with subject [{0}] found!", subject));
		return this;
	}
	...
	public WiserAssertions containsContent(String content) {
		findFirstOrElseThrow(m -> {
			ThrowingSupplier<String> contentAsString = () -> ((String) getMimeMessage(m).getContent()).trim();
			return unchecked(contentAsString).matches(content);
		}, assertionError("No message with content [{0}] found!", content));
		return this;
	}

	private void findFirstOrElseThrow(Predicate<WiserMessage> predicate, Supplier<AssertionError> exceptionSupplier) {
		messages.stream().filter(predicate).findFirst().orElseThrow(exceptionSupplier);
	}
